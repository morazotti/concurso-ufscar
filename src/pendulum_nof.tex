...
\begin{tikzpicture}
    ...
    \draw[-{Stealth[length=3mm,width=2mm]}, thick, back] (3,-3) -- (3,-5) node[right] at (3, -5) {$m\vec{g}$};

    % Decompose the gravity into central and perpendicular components
    \draw[-{Stealth[length=3mm,width=2mm]}, thick, dashed, back] (3,-3) -- (1.5,-4.5) node[midway, left] {$mg \sin\theta$};
    \draw[-{Stealth[length=3mm,width=2mm]}, thick, dashed, back] (3,-3) -- (4,-4) node[midway, right] {$mg \cos \theta$};
    \draw[-{Stealth[length=3mm,width=2mm]}, thick, dashed, back] (3,-3) -- (2,-2) node[midway, above right] {$T$};
    ...
    \draw[back] (3,-3.5) arc (270:315:0.5) node[midway, below, back] {$\theta$};
